    ...
    if (selection && Range.isCollapsed(selection)) {
      ...
      if (blockEntry) {
        ...
        if (headingMatch && headingMatch[2] === " ") {
          ...
          if (Element.isElement(blockNode) && blockNode.type === "heading") {
    ...
    if (selection && Range.isCollapsed(selection)) {
      ...
      if (blockEntry) {
        ...
        if (Point.equals(selection.anchor, start)) {
          if (Element.isElement(blockNode) && blockNode.type === "blockquote") {
        ...
        if (Element.isElement(blockNode) && blockNode.type === "heading") {
          ...
          if (cursorOffset > 0 && currentText[cursorOffset - 1] === "#") {
            ...
            if (hashtagMatch) {
              ...
              if (
                consecutiveHashtags &&
                consecutiveHashtags[1].length === currentHashtagCount
              ) {
                if (currentHashtagCount > 1) {
                  editor.setNodes(
                    {
                      level: (currentHashtagCount - 1) as HeadingLevel
                    },
                    { at: blockPath }
                  );
                } else {
