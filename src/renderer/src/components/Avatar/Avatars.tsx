...
export const Avatars = observer(() => {
  ...
  const showSave =
    (typeof selectedAvatar.avatar === "number" && selectedAvatar.avatar >= 0) ||
            ...
            >
              {[0, 1, 2, 3, 4, 5].map((avatar) => (
                <Paper
                  ...
                  css={{
                    filter:
                      selectedAvatar.avatar === avatar ? `blur(1px)` : "none",
                    boxShadow:
                  ...
                  variant="solid"
                  color={
                    customColor || dynamicElevation(theme.colors.surface, 10)
                  }
                ...
                >
                  <Avatar
                    src={REST.makeCDNUrl(
                      CDNRoutes.defaultUserAvatar(avatar, version)
                    )}
          ...
          >
            {(app.account?.previousAvatars.length ?? 0) > 0 ? (
              app.account?.previousAvatars.map((avatar) => (
                <Box
                  onMouseEnter={() => setFocusedAvatar(avatar)}
                ...
                >
                  {(focusedAvatar === avatar ||
                    ...
                    <IconButton
                      key={`delete-${avatar}`}
                      onClick={() => deletePreviousAvatar(avatar)}
                      css={{
                        position: "absolute",
                        top: 0,
                        right: 0,
